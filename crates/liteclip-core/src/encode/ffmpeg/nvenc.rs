    ...
    pub(super) fn init_nvenc_hardware_encoder(
        ...
        let hw_context = if self.hw_context.is_some() {
            ...
            if ctx.is_shared_device {
                info!(
                    "NVENC resolution change ({}x{}), reusing cached D3D11 device context",
                    out_w, out_h
                );
                ctx.recreate_frames_context(out_w, out_h)?;
                // Take ownership so we can replace self.hw_context later
                // The encoder will hold references via av_buffer_ref
            }
